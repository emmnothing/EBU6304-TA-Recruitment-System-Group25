...
- `ADMINISTRATOR`: view overall system metrics, workload summaries, recent job posts, recent applications, and administrator audit logs
...
- Lets administrators send system announcements to active TA applicants and module organisers
- Lets administrators manage accounts, disable or enable users, reset passwords, and export reporting CSV files
- Lets administrators review audit logs for account changes, announcement sends, CSV exports, and rejected export attempts
...
| `/admin/notifications` | System announcement management | `ADMINISTRATOR` |
| `/admin/audit-log` | Administrator audit log | `ADMINISTRATOR` |
...
- `ADMINISTRATOR`：查看系统总体数据、工作量统计、最近岗位、最近申请记录和管理员审计日志

系统数据统一保存在 `storage/json` 目录下的本地 JSON 文件中，因此无需配置数据库即可完成演示和基本开发， 后续有需求可以加入数据库， 如 SQLite, PostgreSQL, 或者MySQL。
...
- 展示最近发布的岗位和最近的申请动态
- 展示系统当前使用的数据文件位置
- 向活跃 TA 申请人和课程负责人发送系统公告
- 管理用户账号状态、重置密码，并导出 CSV 报表
- 查看账号变更、公告发送、CSV 导出和被拒绝导出请求的审计日志
...
- `storage/json/auditLogs.json`：管理员审计事件
...
| `/admin/users` | 用户账号管理 | `ADMINISTRATOR` |
| `/admin/notifications` | 系统公告管理 | `ADMINISTRATOR` |
| `/admin/audit-log` | 管理员审计日志 | `ADMINISTRATOR` |
